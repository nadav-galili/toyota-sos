...
describe('resolveConflict', () => {
  it('server wins when updatedAt newer', () => {
    const local = { id: 't1', type: 'A', modifiedAt: '2025-01-01T10:00:00Z' };
    const server = { id: 't1', type: 'B', updatedAt: '2025-01-01T11:00:00Z', updatedBy: 'admin' };
    ...
    expect(res.merged.type).toBe('B');
    expect(res.conflict).toBe(true);
    expect(res.ribbon?.updatedBy).toBe('admin');
  });

  it('local wins when modifiedAt newer', () => {
    const local = { id: 't1', type: 'A*', modifiedAt: '2025-01-01T12:00:00Z' };
    const server = { id: 't1', type: 'B', updatedAt: '2025-01-01T11:00:00Z', updatedBy: 'admin' };
    const res = resolveConflict(local, server, { localModifiedAtKey: 'modifiedAt', serverUpdatedAtKey: 'updatedAt', serverUpdatedByKey: 'updatedBy' });
    expect(res.source).toBe('local');
    expect(res.merged.type).toBe('A*');
    expect(res.conflict).toBe(true);
